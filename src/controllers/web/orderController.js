  ...
  try {
    ...
    if (orderList) {
      orderList.productDetails = orderList.productDetails.map((item) => {
        if ('image' in item && item.image && item.image.url.length > 0) {
    ...
    if (orderList.length > 0) {
      for (let doc of orderList) {
        for (let data of doc?.productDetails) {
          if (data && 'image' in data && data.image && data.image.url.length > 0) {
...
export const cancelOrder = async (req, res) => {
  try {
    ...
    if (!id) {
      return sendResponseWithoutData(res, 400, false, 'id value is required!');
    }
    if (!isValidObjectId(id)) {
      return sendResponseWithoutData(res, 400, false, 'Invalid order id!');
    }

    let checkOrderExists = await Order.findOne({ _id: id, status: { $in: ['confirmed', 'pending'] } });
    if (!checkOrderExists) {
      return sendResponseWithoutData(res, 400, false, 'Invalid order id or the order cannot be cancelled!!');

    }

    let checkExists = await Order.findOne({ userId: user._id });

    if (checkExists) {
      let order = await Order.updateOne({ _id: id }, { $set: { status: 'cancelled', reason: reason } });
      if (order) {
