...
export const listState = async (req, res) => {
  try {
    let data = await State.find({ isActive: true }).select('-createdAt -updatedAt -__v -isActive');
...
export const listCity = async (req, res) => {
  try {
    ...
    let data = await City.find(filter).select('-createdAt -updatedAt -__v -isActive');
