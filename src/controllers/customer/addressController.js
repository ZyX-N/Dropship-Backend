...
export const createAddress = tryCatch(async (req, res) => {
    ...
    if (!state || !isValidObjectId(state) || !await detailState_s({ _id: state })) return sendResponseBadReq(res, 'Invalid state!');
    if (!city || !isValidObjectId(city) || !await detailCity_s({ _id: city, state })) return sendResponseBadReq(res, 'Invalid city!');
    if (!pincode || !isValidObjectId(pincode) || !await detailPincode_s({ _id: pincode, city, state })) return sendResponseBadReq(res, 'Invalid pincode!');
...
export const listAddress = tryCatch(async (req, res) => {
    return sendResponseOk(res, 'Address list fetched successfully!', await listAddress_s({ user: req.apiUser._id }, "", [{ path: 'state', select: 'name', match: { isDeleted: false } }, { path: 'city', select: 'name', match: { isDeleted: false } }, { path: 'pincode', select: 'code', match: { isDeleted: false } }]));
});
...
export const detailsAddress = tryCatch(async (req, res) => {
    ...
    return sendResponseOk(res, 'Address details fetched successfully!', await detailAddress_s({ user: req.apiUser._id }, "", [{ path: 'state', select: 'name', match: { isDeleted: false } }, { path: 'city', select: 'name', match: { isDeleted: false } }, { path: 'pincode', select: 'code', match: { isDeleted: false } }]));
});
...
export const updateAddress = tryCatch(async (req, res) => {
    ...
    let newData = {
        state, city, pincode, updatedAt: Date.now()
    }
