...
export const createProduct = tryCatch(async (req, res) => {
  ...
  if (!insertStatus)
    return sendResponseBadReq(res, 'Product insertion failed! try again in sometime or report the issue!');
    ...
    { $match: { isDeleted: false } },
    {
      $lookup: {
        ...
        pipeline: [
          ...
          { $project: { isDeleted: 0, createdBy: 0, updatedBy: 0, createdAt: 0, updatedAt: 0, __v: 0 } },
        ],
      },
    },
    {
...
export const editProduct = tryCatch(async (req, res) => {
  ...
  if (!updateStatus)
    return sendResponseBadReq(res, 'Product updation failed! try again in sometime or report the issue!');

  return sendResponseWithoutData(res, httpStatusCodes.CREATED, true, 'Product updated successfully!');
});

export const deleteProduct = tryCatch(async (req, res) => {
  if (!isValidObjectId(req.params.id) || !(await detailProduct_s({ _id: req.params.id })))
    return sendResponseBadReq(res, 'Invalid product id!');
